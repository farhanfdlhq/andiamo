...
// Helper function untuk membaca cookie (bisa dipindahkan ke file utilitas jika digunakan di banyak tempat)
...
const AdminBatchFormPage: React.FC<AdminBatchFormPageProps> = ({ mode }) => {
  ...
  const handleFormSubmit = async (formData: FormData) => {
    ...
    const fetchHeaders: HeadersInit = { // Buat objek HeadersInit
    ...
    if (xsrfToken) {
        ...
        console.log("AdminBatchFormPage: XSRF Token found and added to headers:", xsrfToken);
    } else {
        console.warn("AdminBatchFormPage: XSRF-TOKEN cookie not found. Proceeding without X-XSRF-TOKEN header.");
    }

    try {
      const response = await fetch(url, {
        method: "POST", // Tetap POST, _method: "PUT" akan ditangani Laravel
        body: formData, //
        headers: fetchHeaders, // Gunakan objek headers yang sudah dimodifikasi
        credentials: 'include', // PENTING untuk mengirim cookie (termasuk session)
      });
      ...
      if (!response.ok) { //
        ...
        if (response.status === 419) {
            errorMessage = "Error: CSRF token mismatch. Silakan refresh halaman dan coba lagi.";
